...
export default class DinoGame extends GameRunner {
  ...
  displayStartMessage() {
    ...
    if (isMobile) {
        startMessage.textContent = "Hmm... you seem a bit lost. Let's play. Tap on the screen";
    } else {
        startMessage.textContent = "Hmm... you seem a bit lost. Let's play. Press space or use up and down arrows.";
    }
    ...
    const startGame = (event) => {
        if (isMobile) {
            ...
            // Detectar la tecla 'Space' en escritorio
            if (event.code === "Space") {
                document.body.removeChild(startMessage);
                this.startGame();
